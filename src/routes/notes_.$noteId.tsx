  ...
  return (
    <NoteShell>
      <main className="mx-auto max-w-5xl px-4 py-6 pb-24 sm:px-6 md:py-8 lg:px-10">
        ...
        {editing ? (
          <div className="mt-4"><h1 className="text-2xl font-semibold text-kumo-strong">Edit Note</h1><LayerCard className="mt-6 flex min-h-[max(36rem,calc(100svh-12rem))] flex-col overflow-hidden p-0"><NoteForm note={note} courses={courses} errors={errors} pending={pending} onCancel={() => void navigate({ search: { mode: "view" } })} onSubmit={update} /></LayerCard></div>
        ) : (
          <>
